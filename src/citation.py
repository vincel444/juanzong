...
# 模型爱给分析句加自造标题，标题不与原文重合会拉低命中率（属于"模型的措辞"
# 而非"引用的证据"）。校验前先剥掉这些前缀，只对实质内容做回查。
#
# 关键约束：**不得误伤合同条款引用**。像"第十二条约定试用期工资按70%执行"里的
# "第十二条"是原文的真实引用，剥掉会显著降低命中率。因此"第X条"仅在后面
# 紧跟冒号/顿号（纯标题形态，如"第十二条：工资约定"）时才剥离。
_PREFIX_RES = [
    # ① 【风险一：xxx】/（一）xxx 方括号标题
    re.compile(r"^[【（(\[][^】）)\]]{1,20}[】）)\]]\s*[:：]?\s*"),
    # ② "风险N：" "要点N：" "问题N：" 等"标签N："
    re.compile(r"^(?:风险|要点|问题|建议|结论|分析|注意|提示|注|说明|综上|总结)"
               r"\s*[一二三四五六七八九十\d]{1,3}\s*[:：、.．]\s*"),
    # ③ "一、" / "1、" / "1." / "1）" 纯序号
    re.compile(r"^[（(]?[一二三四五六七八九十]{1,3}[）)]?\s*[、.．:：]\s*"),
    re.compile(r"^\d{1,2}\s*[、.．:：)]\s*"),
    # ③b 剥完序号后残留的"短标题："—— 只在标题**不含合同实质内容**时剥离。
    #     用词表约束而非泛化正则：泛化的"12字以内+冒号"会误伤
    #     "建议：将试用期修改为不超过二个月"这类有实质内容的句首。
    re.compile(r"^(?:合同期限|试用期|工资|报酬|竞业限制|保密|违约金|违约责任|"
               r"工作内容|工作时间|社会保险|解除条件|争议解决|租金|押金|"
               r"维修责任|转租|质保|知识产权|付款|交付|验收|变更管理|通知)"
               r"[^，。！？；]{0,8}[:：]\s*"),
    # ④ "第X条：" 形式的纯标题（后面**必须**紧跟冒号/顿号，否则视为条款引用保留）
    re.compile(r"^第[一二三四五六七八九十百\d]{1,3}[条款项]\s*[:：、]\s*"),
]


def _strip_prefix(sentence: str) -> str:
    """剥离模型自加的标题/序号前缀，返回实质内容（最多迭代 3 层）。"""
    out = sentence.strip()
    for _ in range(3):
        before = out
        for pat in _PREFIX_RES:
            out = pat.sub("", out, count=1).strip()
        if out == before:
            break
    return out
...
def verify_answer(answer_text: str, index: CitationIndex,
    ...
    搜索兜底。校验前剥离模型自造标题前缀（【风险一：】/一、/1. 等），
    避免"模型的措辞"稀释命中率；展示仍保留原句。

    注意：概括性改写（跳过修饰词）会漏检而标为未溯源，
    属设计取向 —— 宁可误标请人工复核，不可漏掉幻觉。
    """
    items: list[CheckItem] = []
    for sent in split_sentences(answer_text):
        norm = _normalize(_strip_prefix(sent))
